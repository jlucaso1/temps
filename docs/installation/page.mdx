...
This section applies to Linux hosts only -- the settings and files it uses do
not exist on macOS.
...
# always madvise [never]   <- nothing to do (THP off entirely)
```

Only `always` is affected. Under both `madvise` and `never` the kernel stops
handing out huge pages that nothing asked for, which is what causes the
overhead below.
...
Switching to `madvise` cut the resident set by 123 MB, about 28%, against a
run-to-run variance of roughly +/-40 MB on that host. These are two observed
runs, not a repeated measurement, so treat 123 MB as the size of the effect
rather than a saving you can budget for. On that host 190 MB of the 442 MB --
roughly 43% -- was huge pages backing sparsely used allocator regions. A 2-core
VPS sees a much smaller absolute difference, because it uses at most a twelfth
as many arenas. Measure your own host rather than assuming a percentage.
...
and asks for huge pages explicitly instead. Workloads that ask for huge pages
explicitly -- through `madvise(MADV_HUGEPAGE)` or hugetlbfs -- keep getting them
under `madvise`. What you should check before flipping the host is whether
anything on it depends on huge pages it never requested, since that is precisely
what `always` provides and `madvise` withdraws. If nothing does, `madvise` is
the safer default.
...
```bash
# Loop: in the split proxy/console topology both processes are named `temps`,
# so `pidof temps` returns more than one PID.
for pid in $(pidof temps); do
  echo "== pid $pid: $(tr '\0' ' ' < /proc/$pid/cmdline)"
  sudo grep -E 'VmRSS|RssAnon|VmSwap' /proc/$pid/status
  sudo grep AnonHugePages /proc/$pid/smaps_rollup
done
```
...
- Shrinking thread stacks with `RUST_MIN_STACK` does not touch this memory at
  all: glibc already marks every thread stack `MADV_NOHUGEPAGE`, so on a glibc
  build stacks are never huge-page backed (measured on glibc 2.39). It only
  shifts the address-space layout, and has been measured to raise resident
  memory substantially. The musl-based container image was not measured, so on
  that packaging the stack behaviour is untested -- the arena effect above is
  allocator-level and applies either way.
